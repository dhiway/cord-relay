...
# chainspec defaults to cord-local if no arguments are passed to this script;
# if arguments are passed in, the first is the chainspec
chainspec="${1:-cord-local}"
...
# ensure the cord binary exists and is up to date
if [ ! -x "$cord" ] || [ "$cord" -ot "$last_modified_rust_file" ]; then
  cargo build --release
fi
...
# The name must be one of those which can be passed to the cord binary, in un-flagged form,
# one of:

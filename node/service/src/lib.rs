...
//! CORD service. Specialized wrapper over substrate service.
...
pub enum Error {
	...
	#[cfg(feature = "full-node")]
	#[error("Expected a runtime feature")]
	NoRuntime,
}

/// Can be called for a `Configuration` to identify which network the configuration targets.
pub trait IdentifyVariant {
	/// Returns if this is a configuration for the `Cord` network.
...
impl IdentifyVariant for Box<dyn ChainSpec> {
	fn is_cord(&self) -> bool {
		self.id().starts_with("cord") || self.id().starts_with("way")
	}
	fn is_dev(&self) -> bool {
...
fn open_database(db_source: &DatabaseSource) -> Result<Arc<dyn Database>, Error> {
	let parachains_db = match db_source {
		...
		DatabaseSource::Custom { .. } => {
			unimplemented!("No cord subsystem db for custom source.");
		},
	};
	Ok(parachains_db)
}
...
> {
	...
	{
		return chain_ops!(config, jaeger_agent, telemetry_worker_handle; cord_runtime, CordExecutorDispatch, Cord);
	}
...
pub fn build_full(
	...
	#[cfg(feature = "cord-native")]
	{
		return new_full::<cord_runtime::RuntimeApi, CordExecutorDispatch, _>(
			config,
			is_collator,
			grandpa_pause,
			enable_beefy,
		...
		.map(|full| full.with_client(Client::Cord));
	}

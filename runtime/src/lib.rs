...
use runtime_common::{
	assigned_slots, auctions, crowdloan, impl_runtime_weights, paras_registrar, paras_sudo_wrapper,
	prod_or_fast, slots, BlockHashCount, BlockLength, SlowAdjustingFeeUpdate,
};
...
use frame_support::{
	...
	weights::{constants::WEIGHT_PER_MILLIS, ConstantMultiplier},
...
use cord_runtime_constants::{currency::*, fee::*, time::*};
...
parameter_types! {
	...
	pub RuntimeBlockWeights: frame_system::limits::BlockWeights = frame_system::limits::BlockWeights
		::with_sensible_defaults(1000 * WEIGHT_PER_MILLIS, NORMAL_DISPATCH_RATIO);
...
parameter_types! {
	pub const TransactionByteFee: Balance = 100 * MICRO_WAY;
...
parameter_types! {
	...
	pub VotingPeriod: BlockNumber = prod_or_fast!(7 * DAYS, 1 * MINUTES, "CORD_VOTING_PERIOD");
	pub FastTrackVotingPeriod: BlockNumber = prod_or_fast!(3 * HOURS, 1 * MINUTES, "CORD_FAST_TRACK_VOTING_PERIOD");
	pub const MinimumDeposit: Balance = 10 * MILLI_WAY;
	pub EnactmentPeriod: BlockNumber = prod_or_fast!(8 * DAYS, 1, "CORD_ENACTMENT_PERIOD");
	pub CooloffPeriod: BlockNumber = prod_or_fast!(7 * DAYS, 1 * MINUTES, "CORD_COOLOFF_PERIOD");
...
parameter_types! {
	pub const CandidacyBond: Balance = 50 * WAY;
	// 1 storage item created, key size is 32 bytes, value size is 16+16.
	pub const VotingBondBase: Balance = deposit(1, 64);
	// additional data per vote is 32 bytes (account id).
	pub const VotingBondFactor: Balance = deposit(0, 32);
	/// Daily council elections
	pub TermDuration: BlockNumber = prod_or_fast!(30 * DAYS, 2 * MINUTES, "CORD_TERM_DURATION");
...
parameter_types! {
	...
	pub const ProposalBondMinimum: Balance = 2 * MILLI_WAY;
	pub const ProposalBondMaximum: Balance = 5 * WAY;
...
parameter_types! {
	...
	pub const FieldDeposit: Balance = 25 * MILLI_WAY;        // 66 bytes on-chain
	pub const SubAccountDeposit: Balance = 20 * MILLI_WAY;   // 53 bytes on-chain
...
parameter_types! {
	pub const ConfigDepositBase: Balance = 50 * MILLI_WAY;
	pub const FriendDepositFactor: Balance = 10 * MILLI_WAY;
	pub const MaxFriends: u16 = 9;
	pub const RecoveryDeposit: Balance = 10 * MICRO_WAY;
}
...
parameter_types! {
	pub const ParaDeposit: Balance = 40 * WAY;
}
...
parameter_types! {
	...
	pub const SubmissionDeposit: Balance = 10 * WAY; // ~ 10 KSM
	pub const MinContribution: Balance = 1_000 * MILLI_WAY; // ~ .1 KSM
...
construct_runtime! {
		...
		UncheckedExtrinsic = UncheckedExtrinsic
	{
		...
		ValidatorManager: validator_manager::{Pallet, Call, Storage, Event<T>} = 8,
...
mod benches {
	define_benchmarks!(
		...
		[runtime_common::paras_registrar, Registrarparas_sudo_wrapper]
